...
export default function Butterflies() {
  // Desktop butterflies — spread across the full screen
  const desktopButterflies = [
  ...
  // Mobile butterflies — only 3, kept well within screen bounds
  const mobileButterflies = [
    { top: '15%', right: '6%',  size: 26, color: '#F48FB1', delay: 1.0 },
    { top: '55%', right: '4%',  size: 22, color: '#D8B4E2', delay: 1.8 },
    { top: '30%', left:  '4%',  size: 20, color: '#D173C0', delay: 0.6 },
  ];

  return (
    <div className="fixed inset-0 pointer-events-none z-[0] overflow-hidden">
      {/* Desktop butterflies — hidden on mobile */}
      {desktopButterflies.map((b, i) => (
        <motion.div
          key={`desktop-${i}`}
          ...
          animate={{ y: [0, -30, 0], x: [0, 15, 0], rotate: [-5, 5, -5] }}
      ...
      {/* Mobile butterflies — shown only on small screens */}
      {mobileButterflies.map((b, i) => (
        <motion.div
          key={`mobile-${i}`}
          ...
          animate={{ y: [0, -12, 0], x: [0, 6, 0], rotate: [-4, 4, -4] }}
        ...
        >
          ...
          >
            <ButterflySVG size={b.size} color={b.color} className="opacity-70" />
          </motion.div>
        </motion.div>
      ))}
    </div>
